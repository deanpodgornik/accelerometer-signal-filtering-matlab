...
hitrost_raw_just_IIR_without_kalman = zeros(1,length(source),1);
...
for i=1:data_length
    ...
    %integracija - hitrost - (v demonstracijske namene)
    if(i-1>0)
        ...
        hitrost_raw_just_IIR_without_kalman(i) = hitrost_raw_just_IIR_without_kalman(i-1) + Integration_step(sourceWithIIR,i,freq,'trapez');
    else
        hitrost_raw_without_IIR(i) = 0;
        hitrost_raw_with_IIR(i) = 0;
        hitrost_raw_just_IIR_without_kalman(i) = 0;
    end
    
    %integracija - hitrost (IIR + Kalman)
    if(i-1>0)
        hitrost_raw(i) = hitrost_raw(i-1) + Integration_step(filteredData,i,freq,'trapez');
        
        %odstranjevanje nizkih vrednostii
...
plot(x, hitrost_raw_just_IIR_without_kalman, 'color', 'green');
plot(x, hitrost_raw_with_IIR, 'color', 'blue');
legend('Hitrost iz pospeška filtriranega s Kalman filtrom', 'Hitrost iz pospeška filtriranega z IIR filtrom', 'Hitrost iz pospeška filtriranega z IIR + Kalman filtrom');
%ylim([-2 2])
ylim([-1.5 1.5])
grid on
hold off;
